...
class PIDThickenerPressureSimulation:
    ...
    def __init__(self,
        ...
        self.array_scalar = arrayScalar(*self.df_scalar.get_stats())
        # TimeSeries时间戳
    ...
    def step(self, planning_action):
        ...
        if self.observation_seq is None:
            ...
            for i in range(self.output_chunk_length):
                external_cov[self.input_chunk_length - self.output_chunk_length + i][1] = \
                    self.observation_seq.reshape(15, 1)[i][0]

        # 归一化
        external_input = self.array_scalar.scale_scalar(external_input, 3)

        # numpy扩维转TimeSeries
        external_input = np.tile(external_input, (self.input_chunk_length, 1))

        external_input = TimeSeries.from_times_and_values(
            pd.date_range(start=self.t1, freq='1s', periods=len(external_input)), external_input)
        external_cov = TimeSeries.from_times_and_values(
            pd.date_range(start=self.t1, freq='1s', periods=len(external_cov)), external_cov)

        pred_out_f = self.model.predict(self.output_chunk_length, external_input,
                                        external_cov)  # pred_out 为长度为self.output_chunk_length的TimeSeries
        # pred_out 转array
