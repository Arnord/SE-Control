...
class ThickenerSimulation:
    def __init__(self, random_seed, model, scalar):
    ...
    def simulate(self, external_input, covariable):
        # 归一化
        external_input = self.scalar.scale_scalar(external_input, [0, 1, 2])
        external_input = TimeSeries.from_times_and_values(
            pd.date_range(start=self.start_time, freq='1m', periods=len(external_input)), external_input)
        covariable = TimeSeries.from_times_and_values(
            pd.date_range(start=self.start_time, freq='1m', periods=len(covariable)), covariable)

        pred_out_f = self.model.predict(self.output_chunk_length, external_input,
                                        covariable)  # pred_out 为长度为self.output_chunk_length的TimeSeries
        # pred_out 逆归一化
        pred_out_f = self.scalar.unscale_scalar(pred_out_f.all_values(), [3, 4])
        return pred_out_f
...
def step(self, planning_action):
    ...
    covariable = self.df_in_scale[:30].drop(['out_f', 'fill_round'], axis=1).values  # 取前三十个点的固定cov值（不包括output）
    ...
    if self.observation_seq is None:
        ...
        for i in range(self.output_chunk_length):
            covariable[self.input_chunk_length - self.output_chunk_length + i][1] = \
                self.observation_seq.reshape(15, 1)[i][0]
